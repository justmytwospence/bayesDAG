...
5. **Shape-first glyphs.** The density curve is the primary mark; `interval`/`point` annotations are M2 (not built). The glyph registry core is glyph-agnostic — non-univariate kinds (heatmap/ternary/rose/joint) are first-class.
